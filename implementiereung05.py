...
class Warteschlange:
    ...
    def __init__(self, master):
        ...
        self.delete_bestellung = Button(master=self.master, text="Bestellung Abgeschlossen", font=("Arial", 15, "bold"),
                                        bg="gray", command=self.bestellung_fertig)
        self.delete_bestellung.grid(row=4, column=3, sticky="NSEW", pady=10)

        self.add_Bestellung = Button(master=self.master, text="Bestellung Hinzufügen", font=("Arial", 15, "bold"),
                                     bg="gray", command=self.addbestellung)
        self.add_Bestellung.grid(row=4, column=1, sticky="NSEW", pady=10)

        self.anzeige = Label(master=self.master, text="Es liegen keine Bestellungen vor.", bg="gray",
                             font=("Arial", 15, "bold"))
        self.anzeige.grid(row=1, column=1, columnspan=3, rowspan=2, sticky="NSEW")

        self.left = Button(master=self.master, text="←", font=("Arial", 15, "bold"), bg="gray", height=1, width=1,
                           command=self.left)
    ...
    def bestellung_fertig(self):
        if self.checkwarteschlange():
            ...
            self.delete_bestellung.config(bg="red")
            self.master.update_idletasks()
            sleep(0.2)
            self.delete_bestellung.config(bg="grey")
...
itemlist = ["Hamburger", "Cheeseburger", "Bacon-Burger", "Chickenburger", "French Fries", "Chicken Wings",
            "Chicken Nuggets", "Hot Dog", "Root Beer", "Fanta", "Coca Cola", "Sprite"]
